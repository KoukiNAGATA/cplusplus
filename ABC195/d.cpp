...
int main(){
    ...
    REP(i, q){ 
        ...
        vector<int> tmp_x = x;
        cin >> tmp1 >> tmp2;
        FOR(j, tmp1-1, tmp2-1) tmp_x[j] = 0; // 使えない箱は抜くのではなく0にするとO(1)
        sort(tmp_x.begin(), tmp_x.end()); // 小さい順
        for(int j : tmp_x) { // mケの箱を小さい順に
            REP(k, n) { // nケの荷物を価値の高い順に
                if(j >= item[k].first && flag[k] == 0){ //箱の大きさが荷物の大きさ以上かつ、まだ入れられていない荷物
                    ans += item[k].second;
                    flag[k] = 1; // もう荷物を入れた
                    break;
                }
