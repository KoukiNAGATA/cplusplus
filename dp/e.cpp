...
int main()
{
    ...
    { //配列の宣言では個数を宣言するので、0オリジンならばMAX_Nを含んではいけない
    ...
    for (int i = 0; i < n; ++i)
    { //荷物の番目を進めていく。荷物0個の場合があるから0〜nやで！
        for (int sum_v = 0; sum_v < MAX_V; ++sum_v)
        { //荷物の重さ上限を上げていく。価値の総和がsum_vとなるときの重さの最小値を求めるDPをしている
            if (sum_v - v[i] >= 0)
            {                                                                         //v[i]がsum_v内に収まる場合、i番目の品物を選ぶ
    ...
    {
        if (dp[n][sum_v] <= W)
        { //<=Wを満たしていれば、sum_vはループで単調増加するからそこだけ見れば良い
            res = sum_v;
        }
    }
    cout << res << "\n";
