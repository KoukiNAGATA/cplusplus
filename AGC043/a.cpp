...
void flip(int a, int b, Grid grid, int h, int w, int ans)
{ //右としたの連続する黒目を探してまとめてひっくり返す
    int h1, w1 = 0;
...
int maze_count(int a, int b, Grid grid, int h, int w, int ans)
{ //そのマスからゴールまでにひっくり返した回数を返す
    if (grid[a][b])
    { //そのマスが通れる
        if (a == h && b == w)
        { //終了条件
